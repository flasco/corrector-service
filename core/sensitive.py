...
def init_sensitive():
    ...
    keywords = []
    files = glob.glob('./sensitive-repo/*.txt')

    for file in files:
        with open(file, 'rb') as f:
            ks = f.read().decode('utf-8', errors='ignore').splitlines()
            keywords.extend(ks)
            f.close()
    ...
    print(f'sensitive init succeed, len: {len(keywords)}')

def check_sensitive(cur_text):
    global search
    return search.FindAll(cur_text)
